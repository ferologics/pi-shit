...
async function hasLazygit(): Promise<boolean> {
    try {
        await execFileAsync("which", [LAZYGIT_COMMAND], {
            env: process.env,
        });
        return true;
    } catch {
...
function ensureGhosttyEnvironment(): string | null {
    if (process.platform !== "darwin") {
        return "pi-ghostty-lazygit supports Ghostty AppleScript on macOS only.";
    }

    if (!process.env.GHOSTTY_RESOURCES_DIR && process.env.TERM_PROGRAM !== "ghostty") {
        return "pi-ghostty-lazygit must be run from a Ghostty session.";
    }

    return null;
}

export default function piGhosttyLazygitExtension(pi: ExtensionAPI): void {
    pi.registerCommand("lg", {
        description: "Open lazygit in a new Ghostty window at the repo root",
        handler: async (_rawArgs, ctx) => {
            ...
            if (!(await hasLazygit())) {
                ctx.ui.notify("lazygit is not available in PATH.", "error");
                return;
            }

            const repoRoot = await getRepoRoot(ctx);
            if (!repoRoot) {
                ctx.ui.notify("/lg must be run inside a git repository.", "error");
                return;
            }

            try {
                await runAppleScript(buildAppleScript(repoRoot, LAZYGIT_COMMAND));
                ctx.ui.notify("Opened lazygit in a new Ghostty window.", "info");
            } catch (error) {
